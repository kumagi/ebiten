...
func TestQueuedKeyEventsAreStampedAtDrain(t *testing.T) {
	...
	// A tick boundary passes while the browser delivers a full tap (down then up) of Key A. As in
	// the game loop, the input clock is advanced to the tick that reads the events (#3317).
...
func TestQueuedKeyEventsAreAppliedInOrder(t *testing.T) {
	...
	// press because the press comes after it. As in the game loop, the input clock is advanced to
	// the tick that reads the events (#3317).
	u.advanceInputTimeToNextTick()
	u.incrementTick()
	u.enqueueKeys(KeyA, -1, true)
	u.enqueueKeys(KeyA, -1, false)
